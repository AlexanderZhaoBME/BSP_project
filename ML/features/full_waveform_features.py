...
在原代码基础上在I/O部分加以部分改动，并且增加了calculate_rpeak_features与calculate_heart_rate_features两部分新的特征工程
"""
...
class FullWaveformFeatures:
    ...
    def __init__(self, ts, signal_raw, signal_filtered, rpeaks, templates_ts, templates, fs):
        ...
        self.fs = fs
        self.rpeaks = biosppy.signals.ecg.hamilton_segmenter(signal=self.signal_filtered, sampling_rate=self.fs)['rpeaks']
        self.diff_rpeaks = np.diff(rpeaks)
    ...
    def extract_full_waveform_features(self):
        ...
        self.full_waveform_features.update(self.calculate_rpeak_features())
        self.full_waveform_features.update(self.calculate_heart_rate_features())
    ...
    def calculate_rpeak_features(self):
        rpeak_features = dict()

        mean_diff_rpeaks = np.mean(self.diff_rpeaks)
        std_diff_rpeaks = np.std(self.diff_rpeaks)
        rpeak_features['rpeak_diff_mean'] = mean_diff_rpeaks
        rpeak_features['rpeak_diff_std'] = std_diff_rpeaks
        return rpeak_features

    def calculate_heart_rate_features(self):
        heart_rate_features = dict()

        avg_heart_rate = 60 * len(self.rpeaks) * self.fs / (len(self.signal_filtered) - 1)
        std_heart_rate = np.std(60 * self.fs / self.diff_rpeaks)
        heart_rate_features['avg_heart_rate'] = avg_heart_rate
        heart_rate_features['std_heart_rate'] = std_heart_rate
